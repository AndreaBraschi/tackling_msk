function [y] = eval_spline_col(cs, x, indices, dt, flag)
...
if flag == 1
    y.vel = cubicSpline_vel_col(cs, indices, dt);
elseif flag == 2
    y.vel = cubicSpline_vel_col(cs, indices, dt);
    y.acc = cubicSpline_acc_col(cs, indices, dt);
end

end


function vel = cubicSpline_vel_col(cs, indices, dt)
% Analytical expressions:
%   1st derivative: y_dot = 0*x^3 + 3*a1*x^2 + 2*a2*x + a3
...
function acc = cubicSpline_acc_col(cs, indices, dt)
% Analytical expressions:
%   2nd derivative: y_dot_dot: 0*x^3 + 0*a1*x^2 + 6*a1*x + 2*a2
