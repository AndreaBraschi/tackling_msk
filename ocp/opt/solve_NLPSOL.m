function [w_opt,stats, g_opt, lambda_x, lambda_g] = solve_NLPSOL(opti,optionssol)
% --------------------------------------------------------------------------
% solve_NLPSOL
...
%% evaluate cost at guess before solving
f_check = Function('f_check', {opti.x}, {opti.f});
cost_at_guess = full(f_check(guess));
fprintf('Cost at initial guess (IPOPT x0): %f\n', cost_at_guess);
fprintf('guess min: %f, max: %f\n', min(guess), max(guess));
fprintf('guess has NaN: %d, has Inf: %d\n', any(isnan(guess)), any(isinf(guess)));
...
stats = solver.stats();
g_opt = full(sol.g);
lambda_g = full(sol.lam_g);  % Corresponds to constraints (g)
lambda_x = full(sol.lam_x);  % Corresponds to lbx/ubx (design variable bounds)
